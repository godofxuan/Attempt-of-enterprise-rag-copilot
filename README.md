...
|---|---|---|
| External support-KB retrieval | On 200 WixQA ExpertWritten questions, BGE-M3 Dense improved Recall@5 `42.75% -> 66.42%` and nDCG@5 `32.15% -> 52.16%`; p95 `151.8 -> 157.4 ms` | [Aggregate evidence](docs/enterprise_eval/evidence/wixqa_retrieval_baseline_public_v2.json) plus [fresh clean replay](docs/reproduction/evidence/wixqa_clean_reproduction_public_v1.json): `0/63` quality differences at zero tolerance; public-label fixed retrieval, not answer accuracy, blind test, or third-party reproduction |
...
|---|---:|---|
| WixQA enterprise-support retrieval | ExpertWritten `200` authentic anonymized support questions; BM25/Dense/equal-RRF Recall@5 `42.75%/66.42%/59.25%`, nDCG@5 `32.15%/52.16%/47.16%`, p95 `151.8/157.4/304.6 ms`; [final report](docs/enterprise_eval/FINAL_REPORT.md) | Fixed public-label external benchmark, not blind; retrieval only, not answer accuracy; equal RRF rejected |
| WixQA clean replay | Official source re-downloaded into an absent root; 11,975 embeddings and index rebuilt without historical private inputs; all `63` frozen quality comparisons matched exactly at tolerance `0.0`; [public verifier output](docs/reproduction/evidence/wixqa_clean_reproduction_public_v1.json) | Local clean-environment regression replay of consumed public labels; not a new blind holdout or independent third-party reproduction; latency is machine-specific |
| EnterpriseRAG-Bench full-corpus B0 | all `511,962` official rows; verified 1.37 GiB FTS5 index built in `231.35 s` at about `1.83 GiB` peak memory; 470-question Recall@5 `60.37%`, multi-doc complete@5 `28.26%`, p95 `1821.0 ms`; [aggregate evidence](docs/enterprise_eval/evidence/enterprise_rag_bench_bm25_public_v1.json) | Full lexical B0 only; record-aware sensitivity is `60.2677%` because one question uses a reused source ID; Dense/RRF/Agent and answer/citation/refusal quality are `NOT_RUN` |
| WixQA bounded-Agent paired result | same B2 retriever on `400` cases; search `1`, find/open `0`; no retrieval gain; multi-article citation complete `0%`; p95 `1.59x/1.47x`; [aggregate evidence](docs/enterprise_eval/evidence/wixqa_agent_public_v1.json) | `AGENTIC_ROUTE_REJECTED`; mechanism ran without tool errors, but no external Agent quality gain was shown |
| WixQA 20-case multi-document attribution | frozen consumed 60-case subset's 20 multi-document cases; all-gold complete @5/@10/@20 `3/20`, `9/20`, `13/20`; first loss Top-20 `7`, Top-5 `10`, extractive response selection `3`; Gold Retrieval Oracle passed all gold through Guard `20/20` but final complete remained `0/20`; [results](docs/multidoc_attribution/02_RESULTS.md) and [verifiable evidence](docs/multidoc_attribution/evidence/aggregate_v1.json) | `ATTRIBUTION_COMPLETE_NO_OPTIMIZATION`; diagnosis only on a consumed cohort. No serving behavior or answer quality improved; oracle numbers are not resume quality metrics; a future candidate requires a new blind multi-document cohort |
| WixQA bounded multi-document candidate | frozen 20-case four-arm development ablation; combined citation completeness `0% -> 0%`, recall `21.67% -> 24.17%`, precision `45.00% -> 39.17%`, p95 `600.09 -> 1115.59 ms`; [decision](docs/multidoc_candidate/02_RESULTS_AND_DECISION.md) and [verifiable evidence](docs/multidoc_candidate/evidence/aggregate_v1.json) | `DEVELOPMENT_CANDIDATE_REJECTED`; consumed cohort, zero paired complete-case fixes, no fixed validation, serving change, answer-accuracy claim, or resume uplift claim |
| FinQA E19 versioned service wiring | `app.main_v2:app`; 8 OFF/enabled API pairs; response/receipt mismatches `0/0`; OFF worker starts/calls `0/0`; enabled offers/completions `8/8`; provider error isolated; bounded backpressure; residual contexts `0`; [public evidence](docs/external_datasets/evidence/finqa_service_wiring_public_v2.json) | Default Docker entrypoint remains `app.main:app` and FinQA observation remains OFF/0; mechanism and API-contract evidence only, not answer quality, production traffic, SLO, or serving promotion |
...
| FinanceBench external evaluation | pinned open sample: `150` questions, `84` referenced PDFs, `32` companies; company-grouped `49` dev / `101` frozen test; resumable BGE-M3 index `29335 x 1024`; dev dense Page Hit@5 `48.98%`; guarded qwen3 confidence cascade dev Page Hit@5 `53.06%`, Macro Page Recall@5 `46.94%`, `13/49` reranks, mean/p95 `2.46s/5.95s`; frozen v1 test Page Hit@5 `30.69%` | [v2 reranker record](docs/external_datasets/financebench_reranker_v2.md), [external track results](docs/external_datasets/financebench_results.md), and [resume-metrics closeout](docs/resume_metrics/RAG_IMPROVEMENT_REPORT.md); v2 threshold is dev-selected and requires a new independent holdout; old test was not reused; answer accuracy, semantic citation review, and human review are `NOT RUN` |
| UDA FinHybrid external page retrieval | pinned UDA Git/Hugging Face revisions; hash-selected company-disjoint `64` dev / `96` fixed test; BGE-M3 Dense selected by dev nDCG; test Page Hit@5 `73.96%`, MRR@5 `57.07%`, nDCG@5 `61.30%`, p95 `222.91 ms` | [frozen protocol](docs/external_datasets/uda_finance_protocol.md) and [content-free test evidence](docs/external_datasets/evidence/uda_finance_test_v1.json); retrieval is conditioned on the known report; public-label test is consumed; not document discovery, answer accuracy, or baseline-to-improved evidence |
| External retrieved-content security | pinned NVIDIA garak `LatentInjectionReport` combination-disjoint holdout; local Qwen3-8B; Guard OFF/ON ASR `4/12 -> 0/12`, context exposure `12/12 -> 0/12`, benign quarantine `0/2`, mean Guard latency `1.42 ms` | [resume-safe metrics](docs/resume_metrics/RAG_RESUME_METRICS.md) and [aggregate evidence](docs/resume_metrics/evidence/garak_latent_report_holdout_v1.json); one small probe subset, not full garak, arbitrary tool security, or a precise benign FPR estimate |
| R3 external credibility closeout | unused-company UDA page-max validation `Hit@5 81.25% -> 82.29%`, `nDCG@5 67.58% -> 68.46%`: rejected; typed answer dev `7.81% -> 1.56%`: rejected; current-Guard 48-attack garak stress ASR `12/48 -> 0/48`, context exposure `48/48 -> 0/48`, benign quarantine `0/4` | [R3 final report](docs/r3/R3_FINAL_REPORT.md), [offline evidence tour](docs/r3/R3_DEMO_RUNBOOK.md), and [R3 evidence](docs/r3/evidence); fixed tests were not spent on rejected candidates; 48-attack result is recombined stress, not a new blind holdout; independent double-human review remains `NOT_RUN` |
...
| FinQA Gate E18 admitted evidence to typed context | consumes only immutable `AdmittedEvidenceChunk` objects from Agent state; current-Guard rescan, bounded numeric extraction, seven-family bilingual value-free rules, six-state preparation outcomes, safe descriptor catalog, register-before-offer ordering, consume-once handoff and cleanup for unavailable/sample-skip/backpressure/closed outcomes; `112/112` controlled typed builds, local preparation p50/p95/max `0.623/0.921/1.523 ms`, secondary retrieval/model calls `0/0`, enabled admissions/completions `8/8`, default-off worker calls `0`, primary response mismatches `0`, residual workers/contexts `0/0`, 22/22 gates, 25 focused, 61 related and full `3025 passed / 29 skipped`; public audit `1350/0`; first remote run retained a Windows cache-race RED, repair `2a73cbb` passed [exact-SHA CI #54](https://github.com/godofxuan/Attempt-of-enterprise-rag-copilot/actions/runs/30775290120) on Ubuntu, Windows and Linux container in `9m36s` | `E18_ADMITTED_CONTEXT_MECHANISM_PASSED_ROUTE_REMAINS_DISABLED`. The component is injectable but the historical E16 FastAPI assembly is not silently rewritten; E19 must provide versioned route wiring and paired API evidence. These are synthetic mechanism repetitions, not answer accuracy, production traffic, an SLO, arbitrary program coverage or E11 promotion. [E18 record](docs/external_datasets/finqa_admitted_context_gate_e18.md), [learning guide](docs/learning/39_FINQA_GATE_E18_ADMITTED_CONTEXT.md), [handoff](docs/roadmap/finqa_gate_e18_current_handoff.md), and [public evidence](docs/external_datasets/evidence/finqa_admitted_context_public_v1.json) |
...
E18 implements the upstream data-flow mechanism E17 required. It consumes only
the Agent controller's already ACL/Guard-admitted evidence snapshots, performs
zero secondary retrieval and zero model planning, builds a bounded online typed
context, and registers it before E16 admission. Non-admitted offers are cleaned
immediately and observer failures return the exact primary response object.
The normal route remains unchanged pending versioned E19 service wiring. See
the [E18 engineering result](docs/external_datasets/finqa_admitted_context_gate_e18.md).
...
- [FinQA Gate E18 Admitted Context Learning Chapter](docs/learning/39_FINQA_GATE_E18_ADMITTED_CONTEXT.md)
...
- [R3 Final Report](docs/r3/R3_FINAL_REPORT.md)
- [R3 Evidence Index](docs/r3/R3_EVIDENCE_INDEX.md)
- [R3 Engineering Journal](docs/r3/R3_ENGINEERING_JOURNAL.md)
- [R3 Offline Evidence Tour](docs/r3/R3_DEMO_RUNBOOK.md)
