...
| FinanceBench external evaluation | pinned open sample: `150` questions, `84` referenced PDFs, `32` companies; company-grouped `49` dev / `101` frozen test; resumable BGE-M3 index `29335 x 1024`; dev entity-scope document Recall@5 `100%`; selected dense page drilldown Page Hit@5 `48.98%`, complete Page Recall@5 `38.78%`, 98 embedding calls | [External track results](docs/external_datasets/financebench_results.md); first frozen-test attempt failed before result publication on a duplicate same-page snippet precondition and is retained in the journal; raw/private artifacts remain ignored; answer generation/scoring, semantic citation review, and human review are `NOT RUN` |
...
document Recall@5 `100%` from Page Hit@5 `48.98%`, exposing page ranking as the
next quality bottleneck. Frozen test and Qwen answer scores remain `NOT RUN`.
See the
