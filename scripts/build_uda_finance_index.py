...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build the isolated UDA finance index.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        client = OllamaEmbeddingClient.from_settings(
            settings,
            probe_text="UDA finance embedding dimension probe",
            endpoint_context="UDA finance embedding",
        )
        ...
        if embedder.summary is None:
            raise AssertionError("UDA embedding build completed without a cache summary")
        payload = {
