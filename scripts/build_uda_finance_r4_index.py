...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build the isolated UDA finance R4 index.")
    parser.add_argument("--source-root", type=Path, default=R4_SOURCE_ROOT)
    parser.add_argument("--prepared-root", type=Path, default=R4_PREPARED_ROOT)
    parser.add_argument("--output-dir", type=Path, default=R4_PRIVATE_ROOT / "indexes")
    parser.add_argument("--run-id", default="uda-finance-r4-bge-m3-v1")
    ...
    parser.add_argument(
        "--embedding-cache-dir",
        type=Path,
        default=R4_PRIVATE_ROOT / "embedding_cache",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    dataset = verify_uda_finance_r4_preparation(
    ...
    if args.dry_run:
        ...
        payload = {"action": "preview", **preview.model_dump(mode="json")}
    else:
        settings = get_settings()
        client = OllamaEmbeddingClient.from_settings(
            settings,
            probe_text="UDA finance R4 embedding dimension probe",
            endpoint_context="UDA finance R4 embedding",
        )
        ...
        if embedder.summary is None:
            raise AssertionError("R4 embedding build completed without a cache summary")
        payload = {
            "action": "build_and_activate",
            "build_duration_ms": (time.perf_counter() - started) * 1000,
            "embedding_model": client.model_identifier,
            "embedding_model_sha256": client.model_sha256,
            "embedding_cache": embedding_summary_payload(embedder.summary),
...
def embedding_summary_payload(summary: ResumableEmbeddingSummary) -> dict[str, object]:
    return {
        "build_id": summary.build_id,
        "cache_dir": str(summary.cache_dir),
        "total_batches": summary.total_batches,
        "cache_hit_batches": summary.cache_hit_batches,
        "computed_batches": summary.computed_batches,
        "recomputed_batches": summary.recomputed_batches,
