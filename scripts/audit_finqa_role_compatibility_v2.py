try:
    import _bootstrap  # noqa: F401
except ModuleNotFoundError:
    from scripts import _bootstrap  # noqa: F401

import argparse
import hashlib
import json
import shutil
import subprocess
import tempfile
from pathlib import Path

from app.external_datasets.finqa import (
    DEFAULT_PRIVATE_ROOT,
    DEFAULT_SOURCE_ROOT,
    FINQA_DEV_SHA256,
    load_finqa_split,
)
from app.external_datasets.finqa_role_compatibility_audit_v2 import (
    evaluate_role_compatibility_calibration_v2,
)
from app.external_datasets.finqa_role_compatibility_protocol_v2 import (
    load_role_compatibility_protocol_v2,
)
from app.external_datasets.finqa_role_compatibility_v2 import (
    verify_role_exact_parser_enforcement_v2,
)
from app.external_datasets.finqa_semantic_calibration_run import (
    FinQASemanticPlanningCase,
    verify_semantic_calibration_run,
)
from app.external_datasets.finqa_semantic_planning_protocol import (
    load_semantic_planning_protocol,
)
from app.external_datasets.finqa_typed_calibration import case_ids_sha256
from app.external_datasets.finqa_typed_retrospective import (
    canonical_json_bytes,
)
from app.filesystem import atomic_directory_move


REPOSITORY_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_PROTOCOL = (
    REPOSITORY_ROOT
    / "docs"
    / "external_datasets"
    / "evidence"
    / "finqa_role_compatibility_protocol_v2.json"
)
DEFAULT_SOURCE_V1_AUDIT = (
    DEFAULT_PRIVATE_ROOT
    / "role_compatibility_audits"
    / "finqa-role-compatibility-audit-v3"
)
DEFAULT_E5_PROTOCOL = (
    REPOSITORY_ROOT
    / "docs"
    / "external_datasets"
    / "evidence"
    / "finqa_semantic_planning_calibration_protocol_v1.json"
)
DEFAULT_E5_RUN = (
    DEFAULT_PRIVATE_ROOT
    / "semantic_planning_calibration_runs"
    / "finqa-semantic-planning-calibration-v1"
)
DEFAULT_OUT_ROOT = DEFAULT_PRIVATE_ROOT / "role_compatibility_v2_audits"
DEFAULT_PUBLIC_OUTPUT = (
    REPOSITORY_ROOT
    / "docs"
    / "external_datasets"
    / "evidence"
    / "finqa_role_compatibility_v2_calibration_public_v1.json"
)
IMPLEMENTATION_FILES = (
    "app/external_datasets/finqa_typed_program.py",
    "app/external_datasets/finqa_typed_contract_v2.py",
    "app/external_datasets/finqa_typed_contract_v23.py",
    "app/external_datasets/finqa_controlled_program.py",
    "app/external_datasets/finqa_semantic_program_v2.py",
    "app/external_datasets/finqa_role_compatibility_v2.py",
    "app/external_datasets/finqa_role_compatibility_audit_v2.py",
    "app/external_datasets/finqa_role_compatibility_protocol_v2.py",
    "scripts/audit_finqa_role_compatibility_v2.py",
)


def _sha256(path: Path) -> str:
    return hashlib.sha256(path.read_bytes()).hexdigest()


def _git_head() -> str:
    result = subprocess.run(
        ["git", "rev-parse", "HEAD"],
        cwd=REPOSITORY_ROOT,
        check=True,
        capture_output=True,
        text=True,
    )
    revision = result.stdout.strip()
    if len(revision) != 40:
        raise ValueError("Git HEAD is not a full revision")
    return revision


def _load_source_rows(path: Path) -> tuple[FinQASemanticPlanningCase, ...]:
    return tuple(
        FinQASemanticPlanningCase.model_validate(json.loads(line))
        for line in path.read_text(encoding="utf-8").splitlines()
        if line
    )


def _serving_route_disabled() -> bool:
    marker = "finqa_role_compatibility_v2"
    serving_files = [
        path
        for path in (REPOSITORY_ROOT / "app").rglob("*.py")
        if "external_datasets" not in path.parts
    ]
    return all(
        marker not in path.read_text(encoding="utf-8")
        for path in serving_files
    )


def _write_public_once(path: Path, payload: bytes) -> None:
    path = path.resolve()
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.exists():
        if path.read_bytes() != payload:
            raise FileExistsError(
                "public evidence exists with different bytes; use a new version"
            )
        return
    temporary = path.with_name(f".{path.name}.tmp")
    try:
        temporary.write_bytes(payload)
        temporary.replace(path)
    except Exception:
        temporary.unlink(missing_ok=True)
        raise


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the zero-model-call Gate E6-v2 input audit."
    )
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--protocol", type=Path, default=DEFAULT_PROTOCOL)
    parser.add_argument(
        "--source-v1-audit",
        type=Path,
        default=DEFAULT_SOURCE_V1_AUDIT,
    )
    parser.add_argument("--e5-protocol", type=Path, default=DEFAULT_E5_PROTOCOL)
    parser.add_argument("--e5-run", type=Path, default=DEFAULT_E5_RUN)
    parser.add_argument(
        "--dataset",
        type=Path,
        default=DEFAULT_SOURCE_ROOT / "dataset" / "dev.json",
    )
    parser.add_argument("--out-root", type=Path, default=DEFAULT_OUT_ROOT)
    parser.add_argument(
        "--public-output",
        type=Path,
        default=DEFAULT_PUBLIC_OUTPUT,
    )
    return parser


def main(argv: list[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    protocol, protocol_sha256 = load_role_compatibility_protocol_v2(
        args.protocol.resolve()
    )
    source_v1 = args.source_v1_audit.resolve()
    if (
        _sha256(source_v1 / "manifest.json")
        != protocol.source_gate_e6_v1_private_manifest_sha256
        or _sha256(source_v1 / "details.jsonl")
        != protocol.source_gate_e6_v1_private_details_sha256
    ):
        raise ValueError("E6-v2 source audit does not match protocol")
    v1_manifest = json.loads(
        (source_v1 / "manifest.json").read_text(encoding="utf-8")
    )
    if (
        v1_manifest["run_id"] != protocol.source_gate_e6_v1_run_id
        or v1_manifest["protocol_sha256"]
        != protocol.source_gate_e6_v1_protocol_sha256
        or _sha256(source_v1 / "details.jsonl")
        != v1_manifest["artifacts"]["details.jsonl"]
        or _sha256(source_v1 / "summary.json")
        != v1_manifest["artifacts"]["summary.json"]
    ):
        raise ValueError("E6-v2 source audit integrity is invalid")

    e5_protocol, e5_protocol_sha256 = load_semantic_planning_protocol(
        args.e5_protocol.resolve()
    )
    e5_run = args.e5_run.resolve()
    e5_manifest = verify_semantic_calibration_run(
        e5_run,
        protocol=e5_protocol,
        protocol_sha256=e5_protocol_sha256,
    )
    if (
        _sha256(e5_run / "manifest.json")
        != v1_manifest["source_gate_e5_manifest_sha256"]
        or _sha256(e5_run / "details.jsonl")
        != v1_manifest["source_gate_e5_details_sha256"]
    ):
        raise ValueError("E6-v2 transitive Gate E5 source is invalid")
    source_rows = _load_source_rows(e5_run / "details.jsonl")
    if (
        len(source_rows) != protocol.calibration_case_count
        or case_ids_sha256([item.case_id for item in source_rows])
        != protocol.calibration_case_ids_sha256
    ):
        raise ValueError("E6-v2 source cohort does not match protocol")

    cases, dataset_sha256 = load_finqa_split(
        args.dataset.resolve(),
        expected_sha256=FINQA_DEV_SHA256,
    )
    if dataset_sha256 != protocol.development_split_sha256:
        raise ValueError("E6-v2 dataset does not match protocol")
    cases_by_id = {case.id: case for case in cases}
    rows, summary = evaluate_role_compatibility_calibration_v2(
        cases_by_id=cases_by_id,
        source_rows=source_rows,
        protocol=protocol,
        role_exact_parser_enforcement_verified=(
            verify_role_exact_parser_enforcement_v2()
        ),
        serving_route_disabled_verified=_serving_route_disabled(),
    )
    details_bytes = b"".join(
        canonical_json_bytes(row.model_dump(mode="json"), newline=True)
        for row in rows
    )
    summary_bytes = canonical_json_bytes(
        summary.model_dump(mode="json"),
        newline=True,
    )
    implementation_sha256 = {
        relative: _sha256(REPOSITORY_ROOT / relative)
        for relative in IMPLEMENTATION_FILES
    }
    manifest = {
        "schema_version": "finqa_role_compatibility_v2_audit_manifest_v1",
        "run_id": args.run_id,
        "protocol_id": protocol.protocol_id,
        "protocol_sha256": protocol_sha256,
        "source_gate_e6_v1_run_id": v1_manifest["run_id"],
        "source_gate_e6_v1_manifest_sha256": _sha256(
            source_v1 / "manifest.json"
        ),
        "source_gate_e5_run_id": e5_manifest.run_id,
        "source_gate_e5_manifest_sha256": _sha256(
            e5_run / "manifest.json"
        ),
        "dataset_sha256": dataset_sha256,
        "execution_code_revision": _git_head(),
        "implementation_file_sha256": implementation_sha256,
        "artifacts": {
            "details.jsonl": hashlib.sha256(details_bytes).hexdigest(),
            "summary.json": hashlib.sha256(summary_bytes).hexdigest(),
        },
        "summary": summary.model_dump(mode="json"),
    }
    manifest_bytes = canonical_json_bytes(manifest, newline=True)

    out_root = args.out_root.resolve()
    final = out_root / args.run_id
    if final.exists():
        raise FileExistsError("E6-v2 audit exists; use a new run ID")
    out_root.mkdir(parents=True, exist_ok=True)
    staging = Path(tempfile.mkdtemp(prefix=".staging-", dir=out_root))
    try:
        (staging / "details.jsonl").write_bytes(details_bytes)
        (staging / "summary.json").write_bytes(summary_bytes)
        (staging / "manifest.json").write_bytes(manifest_bytes)
        atomic_directory_move(staging, final)
    except Exception:
        shutil.rmtree(staging, ignore_errors=True)
        raise

    public = {
        "schema_version": "finqa_role_compatibility_v2_calibration_public_v1",
        "claim_label": protocol.claim_label,
        "run_id": args.run_id,
        "protocol_id": protocol.protocol_id,
        "protocol_sha256": protocol_sha256,
        "source_gate_e6_v1_public_sha256": (
            protocol.source_gate_e6_v1_public_sha256
        ),
        "private_manifest_sha256": hashlib.sha256(
            manifest_bytes
        ).hexdigest(),
        "private_details_sha256": hashlib.sha256(details_bytes).hexdigest(),
        "summary": summary.model_dump(mode="json"),
        "content_exclusions": [
            "case_ids",
            "questions",
            "answers",
            "gold_program_text",
            "gold_role_values",
            "evidence_text",
            "candidate_ids",
            "compatibility_matrix_rows",
        ],
        "limitations": [
            "disclosed 60-case development calibration only",
            "oracle gold programs label capability and evidence-role recall",
            "zero model calls; answer accuracy was not remeasured",
            "fallback routing does not prove B0 answer correctness",
            "internal validation and frozen test remain unconsumed",
        ],
        "next_action": (
            "RUN_GATE_E6_V2_LIVE_ABLATION"
            if summary.decision == "INPUT_GATE_PASSED"
            else "REVISE_E6_V2_BEFORE_MODEL_CALLS"
        ),
    }
    _write_public_once(
        args.public_output,
        canonical_json_bytes(public, newline=True),
    )
    print(
        json.dumps(
            {
                "run_dir": str(final),
                "public_output": str(args.public_output.resolve()),
                "decision": summary.decision,
                "runtime_capability_route_accuracy": (
                    summary.runtime_capability_route_accuracy
                ),
                "typed_eligible_case_rate": (
                    summary.typed_eligible_case_rate
                ),
                "evidence_role_source_recall": (
                    summary.evidence_role_source_recall
                ),
                "controlled_constant_recall": (
                    summary.controlled_constant_recall
                ),
                "evidence_role_recall_at_4": (
                    summary.evidence_role_recall_at_4
                ),
                "evidence_role_recall_at_8": (
                    summary.evidence_role_recall_at_8
                ),
                "complete_typed_case_rate_at_8": (
                    summary.complete_typed_case_rate_at_8
                ),
                "model_call_count": summary.model_call_count,
            },
            ensure_ascii=False,
            indent=2,
            sort_keys=True,
        )
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
