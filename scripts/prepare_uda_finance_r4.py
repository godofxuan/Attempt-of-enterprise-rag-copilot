...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Prepare the frozen UDA FinHybrid R4 cohort.")
    parser.add_argument("--qa-path", type=Path, required=True)
    parser.add_argument("--archive-path", type=Path)
    parser.add_argument("--pdf-root", type=Path, default=R4_PRIVATE_ROOT / "selected_pdfs")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.verify_only:
        manifest = verify_uda_finance_r4_preparation(
            source_root=args.source_root,
            prepared_root=args.prepared_root,
        )
        payload = {"action": "verify", "status": "PASSED", **manifest.model_dump(mode="json")}
    else:
        protocol, _ = load_uda_finance_r4_protocol(args.protocol)
        ...
        if missing:
            if args.archive_path is None:
                raise FileNotFoundError("R4 selected PDFs are missing; provide --archive-path")
            if pdf_root.exists() and any(pdf_root.iterdir()):
                raise FileExistsError(
                    "R4 selected PDF directory is partial; refusing mixed extraction"
                )
            extract_selected_pdfs(args.archive_path, pdf_root, wanted)
        manifest = prepare_uda_finance_r4(
        ...
        verified = verify_uda_finance_r4_preparation(
            source_root=args.source_root,
            prepared_root=args.prepared_root,
        )
        if manifest != verified:
            raise AssertionError("R4 preparation changed during verification")
