...
def test_e16_public_evidence_binds_protocol_sources_and_implementation() -> None:
    protocol, protocol_sha256 = load_dark_observation_service_protocol_v1(PROTOCOL)
    ...
    assert payload["source_binding"]["source_e15_public_evidence_sha256"] == _sha256(
...
def test_e16_public_evidence_passes_frozen_mechanism_gates() -> None:
    ...
    assert failure["backpressure"]["admitted_count"] == failure["backpressure"]["terminal_count"]
...
def test_e16_public_evidence_contains_no_request_level_or_sensitive_content() -> None:
    ...
    assert not _all_keys(payload).intersection(protocol.public_output.prohibited_content)
    for forbidden in (
...
def test_historical_e16_public_evidence_remains_parseable() -> None:
    payload = json.loads(HISTORICAL_PUBLIC.read_text(encoding="ascii"))

    assert payload["schema_version"] == "dark_observation_service_public_v1"
    assert payload["decision"] == ("E16_MECHANISM_GATE_PASSED_DARK_OBSERVATION_REMAINS_DEFAULT_OFF")
    assert payload["gate_checks"]
    assert all(payload["gate_checks"].values())
