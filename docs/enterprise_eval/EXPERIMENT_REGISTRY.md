...
| ENT-E1-001B | `NOT_FROZEN` | WixQA Simulated + ExpertWritten fixed baseline cohorts | Current bounded search/open policy establishes same-retriever quality/cost baseline | B2/B3 | PROTOCOL_FROZEN_NOT_RUN | `WIXQA_AGENT_PROTOCOL_V1.json`; actual V2 Runner/Registry/Guard, fixed RRF cache, answer correctness not measured |
