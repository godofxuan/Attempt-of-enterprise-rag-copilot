...
| FinQA plan review | 30B proposal + anonymous 8B adjudication 在 100 题 tuning 上 strict `59% -> 63%`，在零重叠 50 题 dev validation 上 `44% -> 50%`、grounded `32% -> 38%`，validation 为 3 修正 / 0 退化 | validation exact McNemar `p=0.25` 未达预冻结 `0.05`；Ollama 0.32.5 CUDA 回归迫使 30B 使用 Vulkan，平均端到端延迟为 baseline 的 `7.84x`；仍是同一数据集 dev，不是独立域 | 默认路径保持关闭；先实现低成本 uncertainty trigger，再在新的冻结 cohort 上验证统计证据、退化率和 CUDA 修复后的真实延迟 |
