...
| `E18-I04` | First remote acceptance passed Ubuntu but failed Windows with `cache_root_unsafe` | Cache-root contents were enumerated before acquiring the cross-process lock; a legitimate writer removed its temp file between enumeration and `lstat()` | Split root-path preparation from root-content validation; validate contents after lock acquisition while retaining hard-link/reparse fail-closed checks | run `30774647704` failure; local repro; `2 passed`; stress `20/20`; full `3025 passed`; replacement run `30775290120` success |
| `E18-I05` | First lock-order repair changed malicious hard-link error from `cache_root_unsafe` to `cache_root_changed` | Structure validation was placed inside the held-directory context, changing exception classification | Move serialized scan after lock acquisition but before held-root identity context | hard-link + concurrency regression `2 passed` |
