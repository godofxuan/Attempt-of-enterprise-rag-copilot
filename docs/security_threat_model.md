...
```mermaid
flowchart LR
    U["Caller"] -->|"HTTP + claimed UserContext"| API["FastAPI boundary"]
    API -->|"validated request"| A["Bounded Agent"]
    ...
    API -->|"hashes only"| DB["SQLite feedback_events"]
    API -->|"safe metadata only"| OBS["In-memory trace + metrics"]
```

当前最重要的边界是：HTTP 请求里的 `UserContext` 经过 Pydantic 结构验证，但仍是调用方自报身份。它只适合本地演示和策略逻辑验证，不是真实认证。生产系统必须由受信任网关或 IAM 从 token/session 注入身份，不能接受浏览器随意声明 tenant/group。
...
| Feedback | 新写入只保存 SHA256、helpful、request ID、时间 | `app/db.py` |
...
| 威胁 | 当前控制 | 验证证据 | 剩余风险 |
|---|---|---|---|
| 跨 tenant/region/group 数据泄露 | `AccessPolicy` 在候选生成和 parent expansion 前过滤；拒绝与不存在使用相同公开信息 | `tests/security/test_acl_zero_leak.py`、`test_navigation_zero_leak.py`、`test_agent_trace_zero_leak.py` | `UserContext` 尚未由真实 IAM 签发；单机 metadata 配置错误仍可能授权错误 |
