...
Current state: `RAG_VNEXT_CLOSED`. The current recommended Chinese wording is
`FINAL_RESUME_ENTRY_CN.md`; this remains a larger candidate pool. Select at most
three primary bullets plus two backups for one target role, and resolve every
number through `../RESUME_METRIC_LEDGER.md`.

## Version A: AI Application / RAG / Agent

Primary:

1. 抽象统一 `AgentOrchestrator`，使默认 bounded controller 与真实 LangGraph StateGraph alternative 共用 ToolGateway、身份/ACL、检索内容准入、Evidence Ledger 与引用过滤；框架不扩权，也不声称提升答案质量。
...
1. 构建覆盖 retrieval、Agent action、citation、security、latency 与失败归因的冻结评测证据链，并定义 hash-linked trajectory、deterministic replay 与 `enterprise.agent-run/1.0` 工件供 EvalOps 做结构和完整性验证。
...
3. 以强类型 Tool Contract 和有状态 ToolGateway 集中执行 identity fingerprint、tenant/ACL narrowing、allow-list、预算与 deadline；官方 MCP SDK 的本地 `search/find/open` 适配仅持 opaque context handle，不能直连存储或绕过 Guard。
