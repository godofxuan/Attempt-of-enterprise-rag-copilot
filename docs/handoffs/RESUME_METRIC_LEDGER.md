...
| VERIFIED_POSITIVE | UDA R4 scoped page-retrieval canary | Hit@5 `76.56% -> 81.25%`; nDCG@5 `64.41% -> 72.61%`; misses `15 -> 12`; p95 `112.65 -> 120.06 ms` | 64 company-disjoint public-label validation questions, known report | Observed page retrieval gain; 6 rescues/3 regressions; post-hoc explicit canary only. Original +5pp Hit gate failed and frozen test was not run | "Improved known-report page retrieval on 64 company-disjoint validation questions and promoted an explicit opt-in canary while preserving the failed original gate" | "Passed the original gate", "test improvement", "answer accuracy", "global production improvement" | `docs/r4/evidence/uda_finance_r4_canary_review_v1.json` |
