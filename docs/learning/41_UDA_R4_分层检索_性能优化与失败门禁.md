...
R4 的 canary 只能说明候选值得小范围尝试，不能说明已经稳定泛化。原因是 64 题中
每题占 1.5625pp，而且 Hit@5 的 bootstrap 区间跨 0。R5 没有运行被旧门禁锁住的
R4 test，也没有继续使用 R4 validation 调参，而是审计完整 UDA 数据后排除前四轮
已经用过的 96 家公司。

剩余 41 家公司每份报告只有 2-7 题，所以不能硬凑“每家 8 题”。R5 使用全部 41 家、
全部 192 道可用题；候选公式、权重、K、chunk 数和 gate 都在结果产生前提交。每道题
的两个 arm 以 case hash 决定先后顺序，双方共用同一份 15,045-chunk BGE-M3 索引。

最终结果：

| 指标 | Dense | Page fusion | 变化 |
|---|---:|---:|---:|
| Hit@5 | 80.21% | 88.02% | +7.81pp |
| nDCG@5 | 70.95% | 77.60% | +6.65pp |
| MRR@5 | 67.80% | 74.11% | +6.30pp |
| p95 | 130.04 ms | 137.60 ms | 1.058x |

逐题成对结果是 154 题都命中、15 题仅新方案命中、0 题仅旧方案命中、23 题都未
命中。漏检从 38 降到 23，相对减少 39.47%。因为同一公司的多道题并不独立，R5
没有使用普通题级 bootstrap，而是按公司整组重采样；Hit 和 nDCG 增益的 95% 下界
仍分别为 +4.10pp 和 +3.32pp。因此这次不是“看到点估计变好就上线”，而是质量、
置信区间、回退数、embedding 调用数和延迟七项门禁全部通过。

晋升范围仍然有限：服务端必须把 policy ID 分类为 finance known-report，才默认使用
page fusion；未分类政策继续走标准检索。`RETRIEVAL_FINANCE_KNOWN_REPORT_PAGE_FUSION_ENABLED`
是 kill switch。这样做的原因是 focused query 的停用词和页面融合机制是在财报任务上
验证的，不能外推到 HR、法务、Wiki 或开放语料文档发现。

这次最适合面试的主线是：**R4 门禁失败 -> 保留失败记录 -> 小范围 canary -> 新公司
确认集 -> 公司簇统计 -> 零回退且低延迟 -> 有边界晋升**。这比“调参数把分数做高”
更能体现评测纪律、发布工程和工业化判断。
